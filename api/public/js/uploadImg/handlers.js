...
function uploadSuccess(file, serverData) {
	try {
		...
		*/
		// 循环费用
        var imglimit = 1;
		var activeObj =  eval('(' + serverData + ')');
		var code = activeObj.code;
		if(code) {
			var img = activeObj.body.thumbnail0.url;
			if(imglimit > 1) {
				var image = $("#imageView").html();
				image+= "<li><img src='"+img+"'><a style='cursor:pointer' onclick='$(this).parent().remove()'>&nbsp;删除</a><input type='hidden' name='"+imgname+"[]' value='"+img+"'></li>";
			}else{
				var image =  "<li><img src='"+img+"'><a style='cursor:pointer' onclick='$(this).parent().remove()'>&nbsp;删除</a><input type='hidden' name='"+imgname+"[]' value='"+img+"'></li>";
			}
