...
<div style="float:left;width:70%">
    <form action="{{ URL::action(\App\Utils\ConstantUtil::PROJECT_ADMIN.'\ColumnController@add')}}" method="post" enctype="multipart/form-data" class="registerform">
        <div class="form-group width50">
            <label for="exampleInputEmail1">栏目名称：</label><font color="red">*</font>
            <input type="text" class="form-control"  placeholder="栏目名称" name="name" datatype="*1-15"  nullmsg="栏目名称不可为空！" errormsg="请输入1~15个汉字">
        </div>
        <!-- 栏目Log -->
        <div class="form-group width50">
            <label for="exampleInputEmail1">栏目Log：</label>
            @include("vendor.Bootstrap.onceUpload.index" ,["onceUpload_name"=>"title_img"])
        </div>
        <!-- 栏目简介 -->
