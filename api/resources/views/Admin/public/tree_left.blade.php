...
<script>
    ...
    function addHoverDom(treeId, treeNode) {
        ...
        sObj.after(addStr);
        sObj.click(function(){
            window.location.href='{{ URL::action(env('CMS').'\ArticleController@index')}}'+'?id='+treeNode.id;
        });
        var btn = $("#addBtn_"+treeNode.tId);
        if (btn) btn.bind("click", function(){
            window.location.href='{{ URL::action(env('CMS').'\ColumnController@addShow')}}'+'?id='+treeNode.id;
            //alert(treeNode.id);
            /*
        ...
        if (edit_btn) edit_btn.bind("click", function(){
            window.location.href='{{ URL::action(env('CMS').'\ColumnController@editorShow')}}'+'?id='+treeNode.id;
        });

        var remove_btn = $("#removeBtn_"+treeNode.tId);
        if (remove_btn) remove_btn.bind("click", function(){
            if(confirm("是否删除该栏目")){
                window.location.href='{{ URL::action(env('CMS').'\ColumnController@delete')}}'+'?id='+treeNode.id;
            }
    ...
    $(function(){
        ...
        $.ajax({
            type: 'POST',
            url:"{{ URL::action(env('CMS').'\ColumnController@index')}}",
            data: {subsection:'123'},
        ...
        $.ajax({
            type: 'POST',
            url:"{{ URL::action(env('CMS').'\ColumnController@info')}}",
            success: function(data){
                var html="";
                var obj = eval('(' + data + ')') ;
                $.each(obj,function(key,val){
